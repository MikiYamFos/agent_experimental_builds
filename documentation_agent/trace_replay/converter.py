...
def fetch_trace(trace_id: str, client: object) -> TraceData:
    ...
    """
    run_row = client.query_json_rows(sql=f"""
        SELECT
...
def fetch_traces(trace_ids: list[str], client: object) -> dict[str, TraceData]:
    ...
    run_rows = client.query_json_rows(sql=f"""
        SELECT
            trace_id,
            attributes->'pydantic_ai.all_messages' as all_messages,
            attributes->>'gen_ai.usage.input_tokens' as input_tokens,
...
def _extract_output(
    messages: list[ModelMessage], output_type: type[T] | None = None
) -> str | dict | T:
...
@overload
def trace_to_run_result(
    trace: TraceData, output_type: type[T]
) -> AgentRunResult[T]: ...


def trace_to_run_result(
    trace: TraceData, output_type: type[T] | None = None
) -> AgentRunResult:
