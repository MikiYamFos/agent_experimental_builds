...
def test_multiple_tool_calls_in_one_response():
    ...
    original = [
        ...
        ModelResponse(
            parts=[
                ToolCallPart(
                    tool_name="weather", args={"city": "NYC"}, tool_call_id="c1"
                ),
                ToolCallPart(
                    tool_name="weather", args={"city": "LA"}, tool_call_id="c2"
                ),
            ],
            finish_reason="tool_call",
        ),
...
async def test_agent_roundtrip():
    ...
    agent = Agent(
        model=TestModel(custom_output_text="test response"), system_prompt="Be helpful."
    )
...
def test_multi_turn_conversation():
    ...
    original = [
        ...
        ModelResponse(
            parts=[
                ToolCallPart(
                    tool_name="search", args={"q": "Python"}, tool_call_id="c1"
                ),
            ],
            finish_reason="tool_call",
        ),
        ...
        ModelResponse(
            parts=[
                ToolCallPart(
                    tool_name="search", args={"q": "Python details"}, tool_call_id="c2"
                ),
            ],
            finish_reason="tool_call",
        ),
...
def test_extract_output_structured():
    ...
    messages = [
        ...
        ModelRequest(
            parts=[
                ToolReturnPart(
                    tool_name="final_result", content="OK", tool_call_id="c1"
                ),
            ]
        ),
    ]
...
def test_extract_output_structured_with_tools_before():
    ...
    messages = [
        ...
        ModelRequest(
            parts=[
                ToolReturnPart(
                    tool_name="search", content="found cats", tool_call_id="c1"
                ),
            ]
        ),
        ModelResponse(
        ...
        ModelRequest(
            parts=[
                ToolReturnPart(
                    tool_name="final_result", content="done", tool_call_id="c2"
                ),
            ]
        ),
    ]
