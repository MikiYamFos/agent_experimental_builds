...
class SearchTools:
    """Wraps the search index and provides search and get_file methods."""
...
def create_documentation_tools_cached():
    ...
    cache = {"index": index, "file_index": file_index}
    with open(cache_file, "wb") as f:
        pickle.dump(cache, f)
    print(f"Documentation tools cached to {cache_file}.")

    return SearchTools(
        index=index, highlighter=create_highlighter(), file_index=file_index
    )
