...
class SearchIndexTools:
    def __init__(self, index: Any, num_results: int = 5) -> None:
        """
        Initialize search tools around an existing index.

        Args:
            index (Any): Search index object that supports search() and append().
            num_results (int): Number of search results to return for each query.
        """
        self.index = index
